...
for i in range(len_real):
    ...
    act1 = model.forward(inp).flatten(start_dim=0, end_dim=2).detach().numpy()
    # print(act1)
    actFake.append(act1)
    inp = torch.cat((orig[i], real[i]), 1)
    inp = inp.type(torch.FloatTensor)
    act2 = model.forward(inp).flatten(start_dim=0, end_dim=2).detach().numpy()
    actReal.append(act2)
    # print(act1)
    res = calculate_fid( act1,act2)
    print(res)
    # print(act1)
# print(act2)
