...
class HighResolutionNet(nn.Module):
    ...
    def forward(self, x):
        x_base_h, x_base_w = x.size(2), x.size(3)
        ...
        return F.interpolate(x, size=(x_base_h, x_base_w), mode='bilinear')


def _hrnet(arch, pretrained, progress, input_nc=3, output_nc=19, norm_layer=None, **kwargs):
    from functools import partial
    if type(norm_layer) == partial:
            norm_layer_actual = norm_layer.func
    else:
        norm_layer_actual = None

    from .hrnet_config import MODEL_CONFIGS
    model = HighResolutionNet(MODEL_CONFIGS[arch], input_nc, output_nc, norm_layer_actual, **kwargs)
    if pretrained:
...
def hrnet18(pretrained=True, progress=True, input_nc=3, output_nc=19, norm_layer=None, **kwargs):
    r"""HRNet-18 model
    """
    return _hrnet('hrnet18', pretrained, progress, input_nc, output_nc, norm_layer,
                   **kwargs)
